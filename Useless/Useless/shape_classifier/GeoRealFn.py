...
class LightweightShapeClassifier:
    ...
    def preprocess_image_lightweight(self, image_or_path, target_size=(128, 128)):
        """
        Robust, light preprocessing: resize -> gray -> CLAHE -> Gaussian -> Otsu -> small morphology -> edges.
        Returns: gray(uint8), edges(uint8)
        """
        try:
            # Load image if a path, else assume BGR ndarray
            ...
            # Resize first (cheap) then gray
            img_small = cv2.resize(img, target_size, interpolation=cv2.INTER_AREA)
            gray = cv2.cvtColor(img_small, cv2.COLOR_BGR2GRAY)

            # Normalize lighting a bit (CLAHE is fast at this size)
            clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
            gray = clahe.apply(gray)

            # Light blur + Otsu binarization
            blurred = cv2.GaussianBlur(gray, (5, 5), 0)
            _, th = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

            # Tiny morphology to clean specks / fill pinholes
            k = np.ones((3, 3), np.uint8)
            th = cv2.morphologyEx(th, cv2.MORPH_OPEN, k, iterations=1)
            th = cv2.morphologyEx(th, cv2.MORPH_CLOSE, k, iterations=1)

            # Edges from cleaned mask
    ...
    def extract_minimal_features(self, gray, edges):
        """
        Compute light but more discriminative shape features.
        """
        try:
            ...
            if area < 0.01 * area_img:  # ignore tiny blobs
                return np.zeros(22, np.float32)
            ...
            # Vertex angle statistics (helps rectangle vs arbitrary quad)
            def angle(a, b, c):
                ab = a - b
                cb = c - b
            ...
            angle_mean = np.mean(angles) if angles else 0.0
            ...
            # Hu moments (log scale) – 7 values, stable-ish
            ...
            ecc = 0.0
            if len(cnt) >= 5:
                (cx_e, cy_e), (MA, ma), _ = cv2.fitEllipse(cnt)
                a = max(MA, ma) / 2.0
                b = min(MA, ma) / 2.0
                if a > 1e-3:
                    ecc = np.sqrt(1.0 - (b*b)/(a*a))  # 0=circle, →1 elongated

            # Edge density
            edge_density = float(np.count_nonzero(edges)) / area_img

            # Intensity stats (global)
            mean_i = float(np.mean(gray)) / 255.0
            std_i  = float(np.std(gray))  / 255.0

            feats = np.array([
                area / area_img,                # normalized area
                peri / (W + H),                 # rough norm
                circularity,
                aspect,
                extent,
                solidity,
                min(vertices, 10)/10.0,
                w / float(W),
                h / float(H),
                cx / float(W),
                cy / float(H),
                ...
                mean_i,
                std_i,
                *hu.astype(np.float32)          # 7 Hu moments (log)
            ], dtype=np.float32)
    ...
    def simple_knn_classifier(self, X_train, y_train, k=3):
        ...
        self.feature_stats = {
            ...
            'std' : (np.std(X_train, axis=0) + 1e-7).astype(np.float32)
        }

    def predict_knn(self, features, conf_thresh=0.6, ratio_thresh=0.9):
        if getattr(self, 'X_train', None) is None:
            return None, 0.0
        f = self.normalize_features(features.reshape(1, -1)).astype(np.float32)
        d = np.sqrt(np.sum((self.X_train - f) ** 2, axis=1))
        k_idx = np.argsort(d)[:self.k]
        k_labels = self.y_train[k_idx]
        unique, counts = np.unique(k_labels, return_counts=True)
        label_id = unique[np.argmax(counts)]
        confidence = np.max(counts) / float(self.k)

        # distance ratio (top-1 vs top-2) – if too similar → uncertain
        d_sorted = np.sort(d[k_idx])
        if len(d_sorted) > 1:
            ratio = d_sorted[0] / (d_sorted[1] + 1e-7)
    ...
    '''
    def rule_based_2d(self, gray, edges):
        """
        Quick geometric rules that beat ML when clear:
        Returns ('circle'|'triangle'|'square'|'rectangle'|None)
        """
        H, W = gray.shape[:2]

        # 1) Try Hough circles on tiny frame
        small = cv2.resize(gray, (96, 96), interpolation=cv2.INTER_AREA)
        small_blur = cv2.medianBlur(small, 3)
        circles = cv2.HoughCircles(small_blur, cv2.HOUGH_GRADIENT, dp=1.2, minDist=16,
                                   param1=120, param2=18, minRadius=8, maxRadius=46)
        if circles is not None and len(circles[0]) > 0:
            return 'circle'

        # 2) Largest contour & polygon
        contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        if not contours:
            return None
        cnt = max(contours, key=cv2.contourArea)
        ...
        if v == 3:
            return 'triangle'
        if v == 4:
            # Check right angles & aspect for square vs rectangle
            pts = approx[:, 0, :].astype(np.float32)
            def ang(a,b,c):
                ab=a-b; cb=c-b
                denom = (np.linalg.norm(ab)*np.linalg.norm(cb)+1e-6)
            ...
            rightish = np.mean(np.abs(np.array(angs)-90.0)) < 10.0

            x,y,w,h = cv2.boundingRect(pts.astype(np.int32))
            aspect = w / max(h,1)
            if rightish:
                return 'square' if 0.90 <= aspect <= 1.11 else 'rectangle'

        # 3) fallback: circularity heuristic
    ...
    def classify_frame(self, bgr):
        pp = self.preprocess_image_lightweight(bgr)
        if pp is None:
            return None, 0.0, {}
        gray, edges = pp

        # Rule-first (fast & crisp)
        rule = self.rule_based_2d(gray, edges)
        if rule is not None:
            return rule, 0.99, {'mode':'rule'}

        # Fallback to k-NN
        feats = self.extract_minimal_features(gray, edges)
        if feats is None or not np.any(feats):
            return None, 0.0, {'mode':'none'}
        label_id, conf = self.predict_knn(feats, conf_thresh=0.65, ratio_thresh=0.88)
        if label_id is None:
            return None, conf, {'mode':'knn-uncertain'}
        return self.reverse_label_map[label_id], conf, {'mode':'knn'}
...
def main_raspberry_pi():
    """Single-shot capture → classify → save → exit (headless, Pi Zero 2 W friendly)."""
    print("=" * 50)
    print("LIGHTWEIGHT SHAPE CLASSIFIER FOR RASPBERRY PI (single-shot, rule-first)")
    print("=" * 50)
    ...
    if os.path.exists(model_path):
        ...
        print(f"Model memory usage: {clf.get_memory_usage():.1f} MB")
    else:
        print("No model found. (Rules will still run; ML fallback disabled)")

    # Camera
    try:
        picam2 = Picamera2()
        cfg = picam2.create_preview_configuration(main={"format":"XRGB8888","size":(320,240)})
    ...
    try:
        ...
        label, conf, meta = clf.classify_frame(bgr)
        if label is None:
            print(f"Prediction: UNKNOWN  confidence={conf:.3f}  mode={meta.get('mode')}")
        else:
            shape_type = "3D" if label in clf.shape_classes_3d else "2D"
            print(f"Prediction: {label.upper()} ({shape_type})  confidence={conf:.3f}  mode={meta.get('mode')}")

        cv2.imwrite("snapshot.jpg", bgr)
...
'''
def main_raspberry_pi():
    """Single-shot: capture ONE frame, classify it, save snapshot, exit."""
    print("=" * 50)
    print("LIGHTWEIGHT SHAPE CLASSIFIER FOR RASPBERRY PI (single-shot)")
    print("=" * 50)
    ...
    if os.path.exists(model_path):
        ...
        print("No model found. Please train/save a model first.")
        return

    # ---- Camera init (Picamera2 / libcamera) ----
    try:
        picam2 = Picamera2()
        # Low resolution keeps CPU/RAM low on Zero 2 W; bump to (640,480) if you want.
        cfg = picam2.create_preview_configuration(
            main={"format": "XRGB8888", "size": (320, 240)}
        )
        picam2.configure(cfg)
        picam2.start_preview(Preview.NULL)  # headless
        picam2.start()
    ...
    try:
        # ---- Capture one frame ----
        frame = picam2.capture_array()                      # (H,W,4), XRGB8888
        frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGBA2BGR) # to BGR for OpenCV

        # ---- Classify ----
        features = classifier.extract_features(frame_bgr)
        if features is None:
            print("No features extracted from frame.")
        else:
            pred_label, confidence = classifier.predict_knn(features)
            if pred_label is None:
                print("Could not classify the frame.")
            else:
                predicted = classifier.reverse_label_map[pred_label]
                shape_type = "3D" if predicted in classifier.shape_classes_3d else "2D"
                print(f"Prediction: {predicted.upper()} ({shape_type})  confidence={confidence:.3f}")

        # ---- Save snapshot (overwrites on each run) ----
...
def main_raspberry_pi():
    """Main function optimized for Raspberry Pi (Picamera2, headless, memory-friendly)."""
    print("=" * 50)
    print("LIGHTWEIGHT SHAPE CLASSIFIER FOR RASPBERRY PI")
    print("=" * 50)
    print("Optimized for Raspberry Pi Zero 2 W (512MB RAM) | Picamera2 (headless)")

    classifier = LightweightShapeClassifier()

    # Load existing model (recommended on Zero 2 W)
    model_path = 'lightweight_shape_model.pkl'
    if os.path.exists(model_path):
        print(f"Loading existing model: {model_path}")
        classifier.load_model_lightweight(model_path)
        ...
        print("No existing model found. Train on a faster machine or use the quick demo.")
        return

    # -------- Camera init (Picamera2 / libcamera) --------
    try:
        picam2 = Picamera2()
        # Start with a tiny resolution for low CPU/RAM. Bump to (640,480) if you need.
        cfg = picam2.create_preview_configuration(
            main={"format": "XRGB8888", "size": (320, 240)}
        )
        picam2.configure(cfg)
        picam2.start_preview(Preview.NULL)   # headless (no GUI)
        picam2.start()
    ...
    # -------- Save policy (memory-friendly) --------
    save_every_sec = 5.0           # save at most once every N seconds
    jpeg_quality = 80              # 60–85 is a good quality/size tradeoff
    last_save = 0.0

    # -------- Loop control & optional memory reporting --------
    last_report = 0.0  # for periodic memory logs
    frame_i = 0

    print("Starting real-time prediction... (Ctrl+C to stop)")
    try:
        while True:
            # Capture a frame (numpy array, HxWx4, XRGB8888)
            frame = picam2.capture_array()

            # Minimal conversion (RGBA -> BGR) for your OpenCV-based features
            ...
            if features is not None:
                ...
                if pred_label is not None:
                    predicted = classifier.reverse_label_map[pred_label]
                    shape_type = "3D" if predicted in classifier.shape_classes_3d else "2D"
                    print(f"{predicted.upper()} ({shape_type})  conf={confidence:.3f}")
                else:
                    print("Could not classify frame")
            else:
                print("No features extracted")

            # ---- Save one compressed snapshot every N seconds (overwrites file) ----
            now = time.time()
            if now - last_save >= save_every_sec:
                ok, buf = cv2.imencode(".jpg", frame_bgr, [cv2.IMWRITE_JPEG_QUALITY, jpeg_quality])
                if ok:
                    # Overwrite the same file to avoid disk growth
                    with open("snapshot.jpg", "wb") as f:
                        buf.tofile(f)
                    last_save = now

            # ---- Optional: print memory every ~2 seconds if helper exists ----
            frame_i += 1
            if now - last_report >= 2.0:
                if 'get_mem_mb' in globals():
                    mem = get_mem_mb()
                    if mem > 0:
                        print(f"[mem] RSS = {mem:.1f} MB  (frame {frame_i})")
                last_report = now
                gc.collect()  # occasional cleanup

            # Small throttle so CPU doesn’t max out
            time.sleep(0.05)
...
def main_raspberry_pi():
    """Main function optimized for Raspberry Pi (libcamera via Picamera2, headless)"""
    print("=" * 50)
    print("LIGHTWEIGHT SHAPE CLASSIFIER FOR RASPBERRY PI")
    print("=" * 50)
    print("Optimized for Raspberry Pi Zero 2 W (512MB RAM) | Picamera2 (headless)")
    ...
    # Load existing model (recommended on the Zero 2 W)
    model_path = 'lightweight_shape_model.pkl'
    if os.path.exists(model_path):
        print(f"Loading existing model: {model_path}")
        classifier.load_model_lightweight(model_path)
        ...
        print("No existing model found. Please train on a stronger machine or use the quick demo.")
        return

    # -------- Picamera2 setup (libcamera) --------
    try:
        picam2 = Picamera2()
        # Keep resolution small for speed; drop to (320,240) if needed
        cfg = picam2.create_preview_configuration(
            main={"format": "XRGB8888", "size": (640, 480)}
        )
        picam2.configure(cfg)
        # Headless preview (no GUI/X11 required)
        picam2.start_preview(Preview.NULL)
        picam2.start()
        time.sleep(0.2)  # small warm-up
    except Exception as e:
    ...
    try:
        while True:
            # Capture a frame as RGBA-like array
            frame = picam2.capture_array()
            # Convert to BGR for OpenCV-based preprocessing
            frame_bgr = cv2.cvtColor(frame, cv2.COLOR_RGBA2BGR)
            ...
            # Save last frame for inspection (safe on headless)
            try:
                cv2.imwrite("last_frame.jpg", frame_bgr)
            except Exception:
                pass

            # Throttle a bit to keep CPU reasonable on Zero 2 W
            time.sleep(0.05)
...
def main_raspberry_pi():
    ...
    if os.path.exists(model_path):
        ...
        if choice == '2':
            ...
            # Training code from the earlier section
            ...

    # Initialize the camera
    cap = cv2.VideoCapture(0)  # 0 for the first connected camera (Pi Camera)
    if not cap.isOpened():
    ...
    while True:
        ...
        features = classifier.extract_features(frame)

        if features is not None:
            # Predict shape class for the captured frame
            prediction, confidence = classifier.predict_knn(features)
